...
covarianceMatrixTimeSeries <- function(timeseries_dataframe){
  ...
  mv <- tibble(Asset = adjusted_dataframe@data$names,
  ...
  covMatrix <- kable((round(adjusted_dataframe@statistics$Cov, digits = 5)),
                     row.names = TRUE,
...
covarianceMatrix <- function(adjusted_dataframe){
  ...
  covMatrix <- cov(adjusted_dataframe, y = NULL, use = "everything", method = c("pearson"))
...
portfolioOptimization <- function(input_dataframe){
  ...
  # Optimize
  maxret.opt <- optimize.portfolio(dailyResults, maxret.port, optimize_method = "random", 
                                   rp = rportfolios)
